...
class ElasticTransform(object):
    ...
    def __call__(self, sample):
        #print(self.mode)
        if self.mode == 'train':
            image, target, bounds = sample['image'], sample['target'], sample['bounds']
            images = [[image, target, bounds]]
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        if self.mode == 'train' or self.mode == 'test':
            ...
            return {'image':image, 'target':target, 'bounds':bounds}
...
class ABUS_Dataset_2d(data.Dataset):
    ...
    def __init__(self, image_path=None, target_path=None, transform=None, sample_k=100, seed=1, mode='train'): 
    ...
    def __getitem__(self, index):
        if self.mode == 'train' or self.mode == 'test':
            ...
            # load target 
            #if self.mode == 'train':
            #    target = np.ones_like(image, image.dtype)*-1
            #    if index in self.sample_flag:
            ...
            bounds = _target2bounds(target)
            # transform 
            sample = {'image':image, 'target':target, 'bounds':bounds}
